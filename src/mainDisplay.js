...
const mainDisplay = projectId => {
  ...
  const togglePriorityButton = document.querySelector(
    ".toggle-priority-button"
  );
  togglePriorityButton.addEventListener("click", () => {
    const projectIndex = find.findProjectIndex(
      find.findProjectInArray(projectId)
    );
    displayController.allProjects[projectIndex].priority = !displayController
      .allProjects[projectIndex].priority;
